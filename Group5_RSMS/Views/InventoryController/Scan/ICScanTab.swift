            ...
            ZStack(alignment: .bottom) {
                ...
                BarcodeScannerView { scannedValue in
                    withAnimation(.spring(response: 0.5, dampingFraction: 0.8)) {
                        viewModel.didScanBarcode(scannedValue, storeId: appState.selectedStore?.id)
                        showScanResult = true
                    }
                }
                .ignoresSafeArea()
            ...
            VStack(spacing: RSMSTheme.Spacing.xl) {
                   ...
                   let rule = viewModel.currentTaxRule {
                    ...
                    if viewModel.currentStock != nil {
                        ...
                        HStack(spacing: RSMSTheme.Spacing.md) {
                            Button {
                                Task {
                                    await viewModel.logInventoryAction(actionType: "restock", storeId: appState.selectedStore?.id)
                                }
                            } label: { restockButtonLabel }

                            Button {
                                Task {
                                    await viewModel.logInventoryAction(actionType: "sale", storeId: appState.selectedStore?.id)
                                }
                ...
                HStack(spacing: RSMSTheme.Spacing.md) {
                    ...
                    Button {
                        if let sku = viewModel.scannedSKU {
                            Task {
                                await viewModel.didScanBarcode(sku, storeId: appState.selectedStore?.id)
                            }
                        }
                    } label: {
                        HStack(spacing: RSMSTheme.Spacing.sm) {
